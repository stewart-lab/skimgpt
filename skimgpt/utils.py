...
class Config:
    def __init__(self, config_path: str):
        ...
        # EVAL_MODEL is the scoring/classifier model; MODEL is kept as a legacy
        # alias so existing configs and all code that reads config.model keep working.
        self.eval_model = self.global_settings.get(
            "EVAL_MODEL", self.global_settings.get("MODEL")
        )
        self.model = self.eval_model
        # FULL_TEXT_MODEL drives PMC full-text figure transcription + chunking (Gemini).
        ...
        # The full-text figure/chunking pipeline uses Gemini via google.genai,
        # which reads GEMINI_API_KEY from the environment.
    ...
    def _build_llm_client(self) -> openai.OpenAI:
        ...
        if self.model and "gemini" in self.model:
            # Gemini eval models use google.genai (GEMINI_API_KEY in env), not this client.
            return None
    ...
    def full_text(self) -> bool:
        """Whether full-text enrichment from PMC is enabled."""
        return self.global_settings.get("FULL_TEXT", False)

    @property
    def use_hypothesis_for_figures(self) -> bool:
        """Whether figure analysis should include the per-PMID hypothesis."""
    ...
    def _required_secret_keys(self) -> list[str]:
        ...
        elif self.model and "gemini" in self.model:
            keys.append("GEMINI_API_KEY")
        else:
            keys.append("OPENAI_API_KEY")
        # Full-text figure transcription / chunking uses a Gemini model.
        if self.full_text_model and "gemini" in self.full_text_model:

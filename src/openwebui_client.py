#!/usr/bin/env python3
"""
OpenWebUI client for relevance filtering inference.
Uses OpenAI-compatible chat completions API.
Model: lexu14/porpoise1 (3.8B parameter text generation model)
Server: http://llm.xdddev.chtc.io
...
- Drop-in replacement for TritonClient
"""
...
class OpenWebUIClient:
    """Client for sending inference requests to OpenWebUI server.

    This client uses the OpenAI-compatible chat completions API provided by OpenWebUI.
    It implements the same interface as TritonClient for easy swapping between backends.
    ...
    DEFAULT_SERVER_URL = "http://llm.xdddev.chtc.io"
    DEFAULT_MODEL_NAME = "lexu14/porpoise1"
    ...
    def __init__(self, server_url: str = None, model_name: str = None,
                 ...
                 max_tokens: int = None):
        """
        Initialize the OpenWebUI client with connection pooling and retry logic.

        Args:
            server_url: Base URL of the OpenWebUI server (defaults to DEFAULT_SERVER_URL)
        ...
        self.completions_url = f"{self.server_url}/api/chat/completions"
        ...
        logger.debug(f"OpenWebUIClient initialized with connection pool (size={pool_maxsize}), "
                    f"retries={max_retries}, timeouts=({connect_timeout}s, {read_timeout}s)")

    def check_server_health(self) -> bool:
        """Check if the OpenWebUI server is ready by making a simple request."""
        try:
            # OpenWebUI doesn't have a dedicated health endpoint, so we check if the API responds
            # We use a minimal request to test connectivity
            response = self.session.get(
                f"{self.server_url}/api/models",
                headers=self._get_headers(),
                timeout=(self.connect_timeout, 10)  # Short read timeout for health check
            )
            return response.status_code in [200, 401, 403]  # 401/403 means server is up but auth issue
    ...
    def generate(self, text_input: str, stream: bool = False,
        ...
        """
        Send a generation request to the OpenWebUI server using chat completions API.
        ...
        Args:
            ...
            exclude_input_in_output: Whether to exclude the input prompt from output (always True for chat API)

        Returns:
            Dictionary containing the generation response with 'text_output' key,
        ...
        # Construct the request payload using OpenAI chat completions format
        payload = {
            ...
            "temperature": sampling_parameters.get("temperature", self.temperature),
            "top_p": sampling_parameters.get("top_p", self.top_p),
            "max_tokens": sampling_parameters.get("max_tokens", self.max_tokens),
            "stream": False  # Streaming not currently supported
        }
        ...
        try:
            ...
            logger.debug(f"OpenWebUI raw response keys: {result.keys()}")
            logger.debug(f"OpenWebUI raw response (truncated): {str(result)[:500]}")
            ...
            if "choices" in result and len(result["choices"]) > 0:
                ...
                return {"text_output": "", "error": "unexpected_format"}
    ...
    def generate_batch(self, text_inputs: List[str],
        ...
        """
        Send multiple generation requests concurrently to the OpenWebUI server.

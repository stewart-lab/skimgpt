...
class ImageAnalyzer:
    ...
    def __init__(self, secrets=None, max_workers=5, logger=None, model_name="gemini-3-flash-preview"):
        ...
        self.logger.info(f"ImageAnalyzer initialized with {max_workers} workers using model {self.model_name}")
    ...
    def _process_single_image(self, figure, full_text):
        ...
        for attempt in range(max_retries):
            try:
                ...
                self.logger.info(f"Sending transcription request to {self.model_name}...")
                transcription_response = self.client.models.generate_content(
                    model=self.model_name,
                    contents=transcription_contents,
                    config=types.GenerateContentConfig(
                        thinking_config=types.ThinkingConfig(thinking_level="high") # Leverage reasoning
                    )
                )
                ...
                self.logger.info(f"Total processing time for {img_path}: {processing_time:.2f} seconds")
                
                # Store results
                figure["transcription"] = transcription
                figure["enhanced_content"] = transcription # Transcription is the enhanced content now
                ...
                if attempt < max_retries - 1:
                    ...
                    self.logger.error(f"Failed to process figure after {max_retries} attempts: {e}")
                    figure["enhanced_content"] = figure.get('caption', '')
                    return figure
    
    def enhance_figure_descriptions(self, figures, full_text):
        """Generate enhanced descriptions for figures using parallel processing."""
        self.logger.info(f"Starting to process {len(figures)} figures in parallel with Gemini")
        
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            future_to_figure = {
                executor.submit(self._process_single_image, figure, full_text): figure
            ...
            for future in concurrent.futures.as_completed(future_to_figure):
                ...
                try:
                    ...
                    self.logger.error(f"Error processing figure {figure.get('local_path')}: {str(e)}")
                    figure["enhanced_content"] = figure.get('caption', '')

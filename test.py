...
def generateOutput(model: any, config: any, hyp: str, abstract: str) -> json:
  ...
  with user():
      lm += "Hi how are you?" + gen(max_tokens = 1, name = "out")
  ...
  return json.dumps({"response": lm["out"]})
...
def main():
    ...
    model = models.TransformersChat("Open-Orca/Mistral-7B-OpenOrca", trust_remote_code = True)

...
This repository provides tools to SKIM through PubMed abstracts to evalaute hypotheses.
 ...
 2. **Install Dependencies (with conda)**
  ...
  3. **Environment Variables**
     ...
     the environment variables (GEMINI_API_KEY is required when full-text enrichment is enabled):
...
- `EVAL_MODEL`: Model used for evaluation/classification (e.g., `"o3"`). Also accepts the legacy key `MODEL`.
- `FULL_TEXT_MODEL`: Model used for full-text figure transcription and chunking (e.g., `"gemini-3-flash-preview"`).
- `FULL_TEXT`: Enable PMC full-text enrichment of relevant abstracts (e.g., `true`).
- `USE_HYP_FOR_FIGS`: Include the per-PMID hypothesis when transcribing figures (e.g., `true`).
...
- `is_dch`: Boolean flag for Direct Comparison Hypothesis (DCH) mode (e.g., `false`).
